...
void SPH::correct_density_error(float* alpha, float dT, float* scalar_values, Float3s* f_tot, float* k_v_i)
{
	...
	do {
		...
		p_avg = (dens_avg / D_NR_OF_PARTICLES) - C_REST_DENS;
		ny = (1.01*(dens_max - dens_min) );
	} while ( (p_avg - C_REST_DENS) > ny );
	//std::cout << (p_avg ) << " > " << ny << std::endl;
}
...
void SPH::correct_divergence_error(float* k_v_i, float* scalar_values, float* alpha)
{
	...
	float avrg = calculate_stiffness(alpha, &m_particles.vel, &m_particles.pred_vel, &m_particles.pos, m_particles.dens, m_delta_t, k_v_i, m_neighbor_data, scalar_values, m_particles.mass);
	...
	Neighbor_Data* neighbor_data, float* alpha, float* kernel_values)
{
	...
	for (auto particle = 0; particle < D_NR_OF_PARTICLES; ++particle)
	{
		...
		for (auto neighbor = 0; neighbor < nr_neighbors; ++neighbor)
		{
			...
			//Update density
			dens[particle] += kernel_values[ind] * mass;// / (4.f / 3.f * pi * pow(0.01, 3));
